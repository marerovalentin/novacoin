...
static const char *strDNSSeed[][2] = {
    {"node.nodcoin.karelia.pro", "dnsseed.nodcoin.karelia.pro"},
    {"nodcoin.ru", "dnsseed.nodcoin.ru"},
    {"nodcoin.ru", "testseed.nodcoin.ru"},
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. NodCoin is probably already running."), addrBind.ToString().c_str());
        else
