...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #nodcoinTEST2\r");
            Send(hSocket, "WHO #nodcoinTEST2\r");
        } else {
            // randomly join #nodcoin00-#nodcoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #nodcoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #nodcoin%02d\r", channel_number).c_str());
        }
